...
function Critere({ label, value, lowLabel, highLabel, color }: { label: string; value: number; lowLabel: string; highLabel: string; color: string }) {
  return (
    <div>
      ...
      <div className="h-[6px] bg-[#E5E5EA] dark:bg-[#38383a] rounded-full overflow-hidden mb-1">
        <div className="h-full rounded-full transition-all" style={{ width: `${value}%`, backgroundColor: color }} />
      </div>
      <div className="flex items-center justify-between text-[9px] text-[#86868B] dark:text-[#98989D]">
        <span>{lowLabel}</span>
        <span>{highLabel}</span>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 dark:bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#1c1c1e] rounded-[22px] shadow-2xl dark:shadow-black/60 w-full max-w-3xl max-h-[92vh] overflow-hidden flex flex-col border border-transparent dark:border-[#38383a]/60">
        {/* Header */}
        <div className="px-6 py-4 border-b border-[#E5E5EA]/40 dark:border-[#38383a]/60 bg-gradient-to-r from-[#F5F5F7] to-white dark:from-[#161618] dark:to-[#1c1c1e]">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#86868B] dark:text-[#98989D] mt-0.5">Régime TVA : {d.regime_tva || "—"} · Forme : {d.forme_juridique || "—"} · Ancienneté : {d.anciennete_annees || 0} ans</p>
              </div>
            </div>
            <button onClick={onClose} className="w-8 h-8 rounded-full bg-[#F5F5F7] dark:bg-[#2c2c2e] hover:bg-[#E5E5EA] dark:hover:bg-[#38383a] flex items-center justify-center">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div>
            <div className="text-[10px] font-semibold text-[#86868B] dark:text-[#98989D] uppercase tracking-wider mb-3">Caractéristiques du client</div>
          ...
          <div>
            <div className="text-[10px] font-semibold text-[#86868B] dark:text-[#98989D] uppercase tracking-wider mb-2">🔧 Spécialités techniques requises</div>
            <div className="flex flex-wrap gap-1.5">
              {(d.specialites_requises || []).map((s, i) => (
                <span key={i} className="text-[11px] bg-[#007AFF]/8 dark:bg-[#0A84FF]/15 text-[#007AFF] dark:text-[#0A84FF] px-2 py-1 rounded-md font-medium">{s}</span>
              ))}
            </div>
            <div className="mt-2 text-[11px] text-[#86868B] dark:text-[#98989D] bg-[#F5F5F7] dark:bg-[#2c2c2e] rounded-[8px] p-2.5">
              <strong className="text-[#1D1D1F] dark:text-white">Profil collaborateur idéal :</strong> {getProfilIdeal()}
            </div>
          </div>

          {/* Collaborateur affecté + Incompatibilités */}
          <div>
            <div className="text-[10px] font-semibold text-[#86868B] dark:text-[#98989D] uppercase tracking-wider mb-2">👤 Collaborateur affecté</div>
            {agent ? (
              <div className="bg-white dark:bg-[#2c2c2e] rounded-[12px] border border-[#E5E5EA]/40 dark:border-[#38383a]/60 p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-[#86868B] dark:text-[#98989D]">{agent.role} · Stress {agent.stress} · Confiance {agent.confiance_joueur}</div>
                </div>
                {incompat.length === 0 ? (
              ...
              <p className="text-[11px] text-[#86868B] dark:text-[#98989D] italic">Aucun collaborateur affecté.</p>
            )}

            {/* Alertes incompatibilité */}
            {incompat.length > 0 && (
              <div className="mt-2 bg-[#FF3B30]/5 dark:bg-[#FF3B30]/10 border border-[#FF3B30]/15 dark:border-[#FF3B30]/25 rounded-[10px] p-3">
                ...
                <ul className="space-y-0.5">
                  {incompat.map((w, i) => (
                    <li key={i} className="text-[11px] text-[#3a3a3c] dark:text-[#d1d1d6] flex items-start gap-1">
          ...
          {/* Économique */}
          <div>
            <div className="text-[10px] font-semibold text-[#86868B] dark:text-[#98989D] uppercase tracking-wider mb-2">💼 Économique</div>
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-[#F5F5F7] dark:bg-[#2c2c2e] rounded-[10px] p-2.5 text-center">
              ...
              <div className="bg-[#F5F5F7] dark:bg-[#2c2c2e] rounded-[10px] p-2.5 text-center">
                <div className="text-[18px] font-bold text-[#007AFF] dark:text-[#0A84FF] tabular-nums">{d.satisfaction || 0}%</div>
                <div className="text-[9px] text-[#86868B] dark:text-[#98989D]">Satisfaction</div>
              </div>
              <div className="bg-[#F5F5F7] dark:bg-[#2c2c2e] rounded-[10px] p-2.5 text-center">
                <div className="text-[18px] font-bold text-[#1D1D1F] dark:text-white tabular-nums">{d.cas_traites}</div>
                <div className="text-[9px] text-[#86868B] dark:text-[#98989D]">Cas traités</div>
              </div>
            </div>
          </div>
          ...
          {/* Statut dossier */}
          <div>
            <div className="text-[10px] font-semibold text-[#86868B] dark:text-[#98989D] uppercase tracking-wider mb-2">📁 Statut du dossier</div>
            <div className="bg-[#F5F5F7] dark:bg-[#2c2c2e] rounded-[12px] p-3 space-y-2">
              <div className="flex items-center justify-between text-[12px]">
                <span className="text-[#86868B] dark:text-[#98989D]">Phase</span>
                <span className="font-semibold text-[#1D1D1F] dark:text-white">{d.phase}</span>
              </div>
              <div className="flex items-center justify-between text-[12px]">
                <span className="text-[#86868B] dark:text-[#98989D]">État</span>
                <span className={`font-semibold ${d.etat === "perdu" ? "text-[#FF3B30]" : d.etat === "avance" ? "text-[#34C759]" : d.etat === "surveillance" ? "text-[#FF9500]" : "text-[#007AFF] dark:text-[#0A84FF]"}`}>
                  {d.etat === "en_cours" ? "EN COURS" : d.etat === "avance" ? "AVANCÉ" : d.etat === "cloture" ? "CLÔTURÉ" : d.etat === "perdu" ? "PERDU" : "SURVEILLANCE"}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[12px] text-[#86868B] dark:text-[#98989D] w-20">Progression</span>
                <div className="flex-1 h-[4px] bg-[#E5E5EA] dark:bg-[#38383a] rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${d.progression}%`, backgroundColor: "#007AFF" }} />
                </div>
                <span className="text-[12px] font-semibold tabular-nums w-10 text-right text-[#1D1D1F] dark:text-white">{d.progression}%</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[12px] text-[#86868B] dark:text-[#98989D] w-20">Qualité</span>
                <div className="flex-1 h-[4px] bg-[#E5E5EA] dark:bg-[#38383a] rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${d.qualite}%`, backgroundColor: d.qualite >= 70 ? "#34C759" : d.qualite >= 50 ? "#FF9500" : "#FF3B30" }} />
                </div>
                <span className="text-[12px] font-semibold tabular-nums w-10 text-right text-[#1D1D1F] dark:text-white">{d.qualite}%</span>
              </div>
            </div>
          </div>
        </div>
